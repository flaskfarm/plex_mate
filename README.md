...
- 1.2.33 (2026.01.11)
    - 파일 정리: 플렉스 에이전트 라이브러리 관련 수정
        ...
        - 과도한 UPDATE 쿼리 실행 수정
        - 리소스 scheme이 metadata인 경우에만 taggings 테이블 조회
    - 파일 정리: 시즌 번호가 6자리일 경우 에피소드 번호를 인식하지 못하는 문제 수정

- 1.2.32 (2025.11.24)
    - 스캔 오류시 최대 스캔 수가 감소되지 않는 문제 수정

- 1.2.31 (2025.10.28)
    - 파일 정리: TV Show의 고아 파일 처리

- 1.2.30 (2025.09.23)
    - 주기적 스캔: 기준경로(anchor) 옵션 추가
    - DB 툴: 휴지통 표시 제거에 영화 타입의 섹션 포함

- 1.2.29 (2025.09.16)
    - Webhook API 주소 출력 버그 수정

- 1.2.28 (2025.08.21)
    - yaml 크레딧 정보 강제 적용 옵션 추가 (웹훅)

- 1.2.27 (2025.08.18)
    - 쿼리 효율성 개선
    - 인트로는 다르거나 없을때 기록, 크레딧은 없을때만 기록
    - show.yaml refresh 수신시 인트로/크레딧 작업
    - `동일파일 동일메타 확인` 버그 수정
      
- 1.2.26 (2025.08.17)
    - yaml에서 Intro, Credit 정보 읽어서 반영
    - 웹훅 - 기본 에이전트 TMDb 정보 갱신 ID에 추가(sjva 에이전트도 추가해두면 인트로 크레딧 작업만 함)
    - 수동반영은 DB 툴 - 간단 명령 - 기본 에이전트 TMDb 정보 갱신 이용
  
- 1.2.25 (2025.07.27) by soju6jan
    - 추가: DB 툴. 영화 타입 변경 기능

- 1.2.24 (2025.07.23)
    - 수정: 제목 색인 정리시 메타 타입 직접 지정

- 1.2.23 (2025.07.16)
    - 추가: 주기적으로 Plex 휴지통 비우기
    - 수정: tmdbsimple 패키지 설치 누락

- 1.2.22 (2025.07.12)
    - 웹 스캔 쇼 단위 스캔 이스케이프 관련 수정

- 1.2.21 (2025.07.11) by soju6jan
    - 스캔 설정 UI 조정.
...
- 1.2.19 (2025.07.10)
    - 에피소드 썸네일도 tmdb 에서 검색함.
      
- 1.2.18 (2025.07.10)
    - 추가: 웹 스캔 요청 API, vfs/refresh API, 범주 복구 API
    - 수정: vfs/refresh 실행 조건에서 경로 검사 삭제
    - 수정: 기본 에이전트에도 다음 에피소드 캐시 적용
    - 수정: 범주 복구 방식 수정

- 1.2.17 (2025.06.29)
    - 기본에이전트 관련 api 추가 (템포몽키 스크립트) 
  
- 1.2.16 (2025.06.16)
    - TV 파일 정리 예외 처리

- 1.2.15 (2025.06.10)
    - 기본 에이전트 대응(show.yaml 과 tmdb 정보 연동)
      
- 1.2.14 (2025.05.14)
    - 인트로 대상 시간 2시간 -> 6시간 이내
    - 추가된 항목 색인정리(자모분리) 기능 추가
    - 라이브러리 복사2 관련 로그 메시지 추가
